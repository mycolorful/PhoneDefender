...
import per.yrj.model.BlackNumberInfo;
...
public class BlackNumberDao {
	...
	public boolean delete(String number) {
		// 获取到可写的数据库
		SQLiteDatabase db = helper.getWritableDatabase();
		int rownumber = db.delete("blacklist", "number=?",
				new String[] { number });
		if (rownumber == 0) {
			return false;
		} else {
	...
	public boolean changeBlockMode(String number, String newmode) {
		...
		int rownumber =db.update("blacklist", values, "number=?", new String[]{number});
		if (rownumber == 0) {
			return false;
		} else {
			return true;
	...
	public List<BlackNumberInfo> findPart(int pagenumber,int pagesize){
		// 得到可读的数据库
		SQLiteDatabase db = helper.getReadableDatabase();
		Cursor cursor = db.rawQuery("select number,mode from blacklist limit ? offset ?", new String[]{String.valueOf(pagesize),
	...
	public List<BlackNumberInfo> findPart2(int startIndex,int maxCount){
		// 得到可读的数据库
		SQLiteDatabase db = helper.getReadableDatabase();
		Cursor cursor = db.rawQuery("select number,mode from blacklist order by _id desc limit ? offset ?", new String[]{String.valueOf(maxCount),
